...
export const create = (estudiante: Estudiante, callback: Function)=>{
    ...
    db.query(
        queryString,
        [estudiante.cod_e, estudiante.nom_e, estudiante.dir_e, estudiante.tel_e, estudiante.fech_nac, estudiante.tipo_u, estudiante.est_e],
        (err, result)=>{
            if (err){
                callback(err);
            }
...
export const getStudents = (callback: Function) => {
  ...
  export const update = (cod_e: number, estudiante: Estudiante, callback: Function) => {
    ...
    db.query(
        queryString,
        [estudiante.nom_e, estudiante.dir_e, estudiante.tel_e, estudiante.fech_nac, estudiante.tipo_u, estudiante.est_e, cod_e],
        (err, result) => {
            if (err) {
                callback(err);
            }
